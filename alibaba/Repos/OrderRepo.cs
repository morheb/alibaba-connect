...
namespace alibaba.Repos
{
    public class OrderRepo : IOrderRepo
    {
        ...
        public async Task<IEnumerable<DbOrder>> FilterOrders(DbOrderCriteria criteria)
        {
            ...
            try
            {
                orders = await sql.GetListQuery($@"SELECT o.id, o.withDelivery, r.location as restLocation,o.restaurantId,u.phonenumber as phonenumber, o.driverId,o.status,o.price, o.userId, r.name as restaurantName , o.location as location ,o.extraFees ,u.username as username, o.deliveryFees, o.type FROM orders o join users u on o.userid = u.id join restaurants r on r.id =o.restaurantId where {query} LIMIT {criteria.PageSize * (criteria.PageNumber - 1)},{criteria.PageNumber * criteria.PageSize}
